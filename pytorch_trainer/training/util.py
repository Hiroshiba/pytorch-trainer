from pytorch_trainer.training.triggers import interval_trigger
...
def get_trigger(trigger):
    ...
    :class:`~pytorch_trainer.training.Trainer` object as an argument and returns a
    ...
    trigger is passed to the :class:`~pytorch_trainer.training.Trainer` as the `stop
    trigger`, the training loop breaks when the trigger returns True. If the
    trigger is passed to the :meth:`~pytorch_trainer.training.Trainer.extend` method of
    ...
    it passes the value to :class:`~pytorch_trainer.training.triggers.IntervalTrigger`.
    ...
    Args:
        trigger: Trigger object. It can be either an already built trigger
            ...
            passed to :class:`~pytorch_trainer.training.triggers.IntervalTrigger`.

    Returns:
        ``trigger`` if it is a callable, otherwise a
        :class:`~pytorch_trainer.training.triggers.IntervalTrigger`

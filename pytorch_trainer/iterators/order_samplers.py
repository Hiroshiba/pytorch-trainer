...
class ShuffleOrderSampler(OrderSampler):
    ...
    >>> dataset = [(1, 2), (3, 4)]
    >>> it = pytorch_trainer.iterators.MultiprocessIterator(dataset, 1, shuffle=True)
    >>> it = pytorch_trainer.iterators.MultiprocessIterator(
    ...     dataset, 1, order_sampler=pytorch_trainer.iterators.ShuffleOrderSampler())
